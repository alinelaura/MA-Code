...
votertypeYear <- function(year){
  ...
  data_shares_t2 <- data_name %>% 
    mutate(type = "vote_type_det") %>% 
    select(abstimmungsjahr, type, vote_type_det) %>% 
    group_by(abstimmungsjahr, type, vote_type_det) %>% 
    summarise(count = n()) %>%
    ungroup() %>% 
    mutate(share = count/sum(count))
  ...
  rm(data_name, data_shares_t1, data_shares_t2)
  
  # Print result
  share_vote_type
}

data_2013 <- votertypeYear(2013)
data_2014 <- votertypeYear(2014)
data_2015 <- votertypeYear(2015)
data_2016 <- votertypeYear(2016)
data_2017 <- votertypeYear(2017)
data_2018 <- votertypeYear(2018)
data_2019 <- votertypeYear(2019)
data_2020 <- votertypeYear(2020)
...
### Kumulative Anteil von Beteiligungshäufigkeit an Abstimmungen
