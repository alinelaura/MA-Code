...
data_mlogit_h2 <- data_mlogit %>% 
  mutate(steuerb_einkommen_imputed = steuerb_einkommen_imputed/1000,
         massgebendesEinkommen_imputed = massgebendesEinkommen_imputed/1000,
         reinvermoegen_imputed = reinvermoegen_imputed/1000,
         hhaequEinkommen = hhaequEinkommen/1000,
         hhaequVermoegen = hhaequVermoegen/1000) %>% 
...
ologit_income = polr(anz_teilnahme ~ alter_v + sex_c + konfession_c + residenz10 + zugezogen + steuerb_einkommen_imputed, 
                    data = data_mlogit_h2, Hess = TRUE)
summary(ologit_income)

ologit_wealth= polr(anz_teilnahme ~ alter_v + sex_c + konfession_c + residenz10 + zugezogen + reinvermoegen_imputed, 
                   data = data_mlogit_h2, Hess = TRUE)
summary(ologit_wealth)


ologit_incwealth= polr(anz_teilnahme ~ alter_v + sex_c + konfession_c + residenz10 + zugezogen + steuerb_einkommen_imputed + reinvermoegen_imputed, 
                      data = data_mlogit_h2, Hess = TRUE)
...
mlogit_empty <- multinom(vote_type ~ alter_v + sex_c + konfession_c + residenz10 + zugezogen ,
                       data = data_mlogit_h2)
mlogit_inc <- multinom(vote_type ~ alter_v + sex_c + konfession_c + residenz10 + zugezogen + steuerb_einkommen_imputed,
                       data = data_mlogit_h2)
mlogit_wealth <- multinom(vote_type ~ alter_v + sex_c + konfession_c + residenz10 + zugezogen + reinvermoegen_imputed,
                       data = data_mlogit_h2)
mlogit_incwealth <- multinom(vote_type ~ alter_v + sex_c + konfession_c + residenz10 + zugezogen + steuerb_einkommen_imputed + reinvermoegen_imputed,
                       data = data_mlogit_h2)
...
```{r message=FALSE, warning=FALSE}
mlogit_inc2 <- multinom(vote_type ~ age_cat_num + sex_c + konfession_c + residenz10 + zugezogen + steuerb_einkommen_imputed,
                       data = data_mlogit_h2)
mlogit_wealth2 <- multinom(vote_type ~ age_cat_num + sex_c + konfession_c + residenz10 + zugezogen + reinvermoegen_imputed,
                       data = data_mlogit_h2)
...
plot_income <- ggplot(profile_income, aes(x = steuerb_einkommen_imputed_val1, col = age_cat)) +
  geom_line(aes(y = val1_mean), size = 1) +
...
plot_wealth <- ggplot(profile_wealth, aes(x = reinvermoegen_imputed_val1, col = age_cat)) +
  geom_line(aes(y = val1_mean), size = 1) +
  facet_wrap(~ level, ncol = 1) +
  labs(title = "Vermögen",
...
library(ggpubr)
ggarrange(plot_income, plot_wealth, ncol=2, common.legend = TRUE, legend="bottom")

```


## H3: Effects of Jumps in Income / Wealth
...
## H4: Effect of parents income on children (Übertragungshypothese)
...
```{r message=FALSE, warning=FALSE}
# TODO: can I just  define youth by their age group (18-25) living with older (parent) age group (35-65)?
...
data_households <- data %>% 
  ...
  # use household income / wealth to show affluency of family
  filter(young_pers == 1) %>% 
  ungroup() %>% 
