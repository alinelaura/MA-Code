...
data_year <- data_year %>%
  ...
  mutate(time_to_treat = ifelse(treat_less50income == 1, (abstimmungsjahr - treatment_year_inc), 0))
...
```{r}
library(fixest) 
mod_twfe = feols(share_teilnahme ~ i(time_to_treat, treat_less50income, ref = -1) + ## key interaction: time × treatment status
...
iplot(mod_twfe, 
      ...
      main = 'Event-Study of the Effects of Income Shocks on Voter Turnout (TWFE)')
...
Figure 3 shows estimates from within-individual event-study models with controls as in column 4 of Table 1. Error bars show 95% confidence intervals.We provide descriptive statistics about income transitions in the Online Appendix (p. 11).
To gain a deeper understanding of the curvilinear relationship between income and turnout, we now investigate asymmetries between negative and positive income shocks. The left panel in Figure 3 shows the event-study for transitions from some to 0 income. Reassuringly for our identification assumption, treated and control households share statistically indistinguishable voter turnout in pretransition elections (i.e., τ < 0). In contrast, turnout decreases after losing income (i.e., τ ≥ 0) by a significant 5 p.p.
